...
// Note: Position-based equation placement now uses sectionEquationMap for accurate positioning
...
// Generate IEEE Table - Using proven working approach from July 9th
export const generateIEEETable = (table: TableData, config: ProcessingConfig): string => {
  ...
  // EXACT original IEEE format that was working correctly (from July 9th)
...
// Generate ACM Table - Using proven working approach from July 9th
export const generateACMTable = (table: TableData, config: ProcessingConfig): string => {
  ...
  // Calculate proper column width to prevent touching right edge (from July 9th)
...
// Generate Springer Table - Using proven working approach from July 9th
...
export const generateLaTeX = (analysis: Analysis, config: ProcessingConfig): string => {
  ...
  // Sections with tables and equations using placeholder-based placement
  ...
  if (unplacedEquations.length > 0 && analysis.sections.length > 0) {
    // Find the most appropriate section (prefer later sections for mathematical content)
    const targetSectionIndex = Math.min(analysis.sections.length - 1, 
      Math.max(0, analysis.sections.length - 2)); // Use second-to-last or last section
    
    if (analysis.sections[targetSectionIndex]) {
      // Add unplaced equations to the target section as a footnote
      latex = latex.replace(
        new RegExp(`(${escapeRegex(analysis.sections[targetSectionIndex].title)}[\\s\\S]*?)(?=\\\\(?:sub)*section|\\\\end\\{document\\})`),
        (match) => {
          let addendum = match;
