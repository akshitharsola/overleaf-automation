...
// IEEE: Uses \IEEEauthorblock with ordinal numbering and detailed affiliation
// ACM: Uses individual \author commands with shared \affiliation blocks
// Springer: Uses \author* for corresponding authors with \fnm and \sur formatting
...
${email}}`;
    ...
    // ACM: Individual author blocks with separate affiliations (exact template structure)
    let latex = '';
    
    // Generate author blocks following exact ACM template structure
    names.forEach((name, index) => {
      const formattedName = formatAuthorName(name);
      const email = emails[index] || '';
      
      latex += `\\author{${formattedName}}\n`;
      
      // Add authornote for first author
      if (index === 0) {
        latex += `\\authornote{Both authors contributed equally to this research.}\n`;
      }
      ...
      // Add ORCID for first author
      if (index === 0) {
        latex += `\\orcid{1234-5678-9012}\n`;
      }
      
      // Add individual affiliation block for each author
      latex += `\\affiliation{%\n`;
      latex += `  \\institution{${formatUniversity(university)}}\n`;
      latex += `  \\city{Indore}\n`;
      latex += `  \\state{Madhya Pradesh}\n`;
      latex += `  \\country{India}\n`;
      latex += `}\n\n`;
    });
    
    // Add shortauthors command for page headers
    if (names.length > 2) {
      const firstAuthor = names[0].split(' ').pop(); // Get last name
      latex += `\\renewcommand{\\shortauthors}{${firstAuthor}, et al.}\n\n`;
    }
...
// IEEE: Uses |>{centering\arraybackslash}p{width}| with \hline
// ACM: Uses booktabs (\toprule, \midrule, \bottomrule) with proper column widths
// Springer: Uses booktabs with centered layout and compact formatting
...
export const generateLaTeX = (analysis: Analysis, config: ProcessingConfig): string => {
  ...
  if (config.selectedTemplate === 'ieee') {
    ...
    latex += `%%\n`;
    latex += `%% \\BibTeX command to typeset BibTeX logo in the docs\n`;
    latex += `\\AtBeginDocument{%\n`;
    latex += `  \\providecommand\\BibTeX{{%\n`;
    latex += `    Bib\\TeX}}}\n\n`;
    latex += `%% Rights management information.  This information is sent to you\n`;
    latex += `%% when you complete the rights form.  These commands have SAMPLE\n`;
    latex += `%% values in them; it is your responsibility as an author to replace\n`;
    latex += `%% the commands and values with those provided to you when you\n`;
    latex += `%% complete the rights form.\n`;
    latex += `\\setcopyright{acmlicensed}\n`;
    latex += `\\copyrightyear{2018}\n`;
    latex += `\\acmYear{2018}\n`;
    latex += `\\acmDOI{XXXXXXX.XXXXXXX}\n\n`;
    latex += `%%\n`;
    latex += `%% These commands are for a JOURNAL article.\n`;
    latex += `\\acmJournal{TOG}\n`;
    latex += `\\acmVolume{37}\n`;
    latex += `\\acmNumber{4}\n`;
    latex += `\\acmArticle{111}\n`;
    latex += `\\acmMonth{8}\n\n`;
    latex += `%%\n`;
    latex += `%% Submission ID.\n`;
    latex += `%% Use this when submitting an article to a sponsored event. You'll\n`;
    latex += `%% receive a unique submission ID from the organizers\n`;
    latex += `%% of the event, and this ID should be used as the parameter to this command.\n`;
    latex += `%%\\acmSubmissionID{123-A56-BU3}\n\n`;
    latex += `%%\n`;
    latex += `%% The majority of ACM publications use numbered citations and\n`;
    latex += `%% references.  The command \\citestyle{authoryear} switches to the\n`;
    latex += `%% "author year" style.\n`;
    latex += `%%\n`;
    latex += `%% If you are preparing content for an event\n`;
    latex += `%% sponsored by ACM SIGGRAPH, you must use the "author year" style of\n`;
    latex += `%% citations and references.\n`;
    latex += `\\citestyle{acmauthoryear}\n\n`;
...
\\thanks{Identify applicable funding agency here. If none, delete this.}
    ...
    latex += `%%\n`;
    latex += `%% The "title" command has an optional parameter,\n`;
    latex += `%% allowing the author to define a "short title" to be used in page headers.\n`;
    latex += `\\title{${titleText}}\n\n`;
  } else if (config.selectedTemplate === 'springer') {
  ...
  // Add ACM author comments
  if (config.selectedTemplate === 'acm') {
    latex += `%%\n`;
    latex += `%% The "author" command and its associated commands are used to define\n`;
    latex += `%% the authors and their affiliations.\n`;
    latex += `%% Of note is the shared affiliation of the first two authors, and the\n`;
    latex += `%% "authornote" and "authornotemark" commands\n`;
    latex += `%% used to denote shared contribution to the research.\n`;
  }
  ...
  if (analysis.abstract) {
    if (config.selectedTemplate === 'acm') {
      latex += `%%\n`;
      latex += `%% The abstract is a short summary of the work to be presented in the\n`;
      latex += `%% article.\n`;
    }
    latex += `\\begin{abstract}\n${compressText(analysis.abstract.text, config)}\n\\end{abstract}\n\n`;
  }
  
  // Keywords and CCS concepts (ACM specific)
  if (analysis.keywords) {
    if (config.selectedTemplate === 'ieee') {
      latex += `\\begin{IEEEkeywords}\n${compressText(analysis.keywords.text, config)}\n\\end{IEEEkeywords}\n\n`;
    } else if (config.selectedTemplate === 'acm') {
      // Add CCS concepts section (exact template format)
      latex += `%%\n`;
      latex += `%% The code below is generated by the tool at http://dl.acm.org/ccs.cfm.\n`;
      latex += `%% Please copy and paste the code instead of the example below.\n`;
      latex += `%%\n`;
      latex += `\\begin{CCSXML}\n`;
      latex += `<ccs2012>\n`;
      latex += ` <concept>\n`;
      latex += `  <concept_id>00000000.0000000.0000000</concept_id>\n`;
      latex += `  <concept_desc>Do Not Use This Code, Generate the Correct Terms for Your Paper</concept_desc>\n`;
      latex += `  <concept_significance>500</concept_significance>\n`;
      latex += ` </concept>\n`;
      latex += ` <concept>\n`;
      latex += `  <concept_id>00000000.00000000.00000000</concept_id>\n`;
      latex += `  <concept_desc>Do Not Use This Code, Generate the Correct Terms for Your Paper</concept_desc>\n`;
      latex += `  <concept_significance>300</concept_significance>\n`;
      latex += ` </concept>\n`;
      latex += ` <concept>\n`;
      latex += `  <concept_id>00000000.00000000.00000000</concept_id>\n`;
      latex += `  <concept_desc>Do Not Use This Code, Generate the Correct Terms for Your Paper</concept_desc>\n`;
      latex += `  <concept_significance>100</concept_significance>\n`;
      latex += ` </concept>\n`;
      latex += `</ccs2012>\n`;
      latex += `\\end{CCSXML}\n\n`;
      latex += `\\ccsdesc[500]{Do Not Use This Code~Generate the Correct Terms for Your Paper}\n`;
      latex += `\\ccsdesc[300]{Do Not Use This Code~Generate the Correct Terms for Your Paper}\n`;
      latex += `\\ccsdesc{Do Not Use This Code~Generate the Correct Terms for Your Paper}\n`;
      latex += `\\ccsdesc[100]{Do Not Use This Code~Generate the Correct Terms for Your Paper}\n\n`;
      
      // Add keywords
      latex += `%%\n`;
      latex += `%% Keywords. The author(s) should pick words that accurately describe\n`;
      latex += `%% the work being presented. Separate the keywords with commas.\n`;
      latex += `\\keywords{${compressText(analysis.keywords.text, config)}}\n\n`;
      
      // Add received dates
      latex += `\\received{20 February 2007}\n`;
      latex += `\\received[revised]{12 March 2009}\n`;
      latex += `\\received[accepted]{5 June 2009}\n\n`;
    } else {
      latex += `\\keywords{${compressText(analysis.keywords.text, config)}}\n\n`;
    }
  }
  ...
  // Add ACM-specific acknowledgments and bibliography
  if (config.selectedTemplate === 'acm') {
    latex += `\\begin{acks}\n`;
    latex += `To our colleagues and reviewers for their valuable feedback and suggestions.\n`;
    latex += `\\end{acks}\n\n`;
