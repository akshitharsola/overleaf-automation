...
import { Analysis, Section, TableData, DetectedElement, TextLine, Equation, AuthorInfo } from '../types/DocumentTypes';
...
// Parse semicolon-separated author information
const parseAuthorInfo = (text: string): AuthorInfo | null => {
  ...
  let university = '';
  let emails: string[] = [];
  let correspondingAuthors: number[] = [];
  
  for (const line of lines) {
    const lowerLine = line.toLowerCase();
    
    // Names line - starts with "name" or "author"
    if (lowerLine.startsWith('name') || lowerLine.startsWith('author')) {
      const content = line.split(/[:-]/, 2)[1]?.trim();
      if (content) {
        names = content.split(';').map(name => name.trim()).filter(name => name);
      }
    }
    // Department line
    else if (lowerLine.startsWith('department')) {
      const content = line.split(/[:-]/, 2)[1]?.trim();
      if (content) {
        departments = content.split(';').map(dept => dept.trim()).filter(dept => dept);
      }
    }
    // University line
    else if (lowerLine.startsWith('university') || lowerLine.startsWith('institution')) {
      const content = line.split(/[:-]/, 2)[1]?.trim();
      if (content) {
        university = content;
      }
    ...
    // Email line
    else if (lowerLine.startsWith('mail') || lowerLine.startsWith('email')) {
      const content = line.split(/[:-]/, 2)[1]?.trim();
      if (content) {
        const emailList = content.split(';').map(email => email.trim()).filter(email => email);
        emails = emailList.map(email => email.replace('*', '').trim());
        
        // Find corresponding authors (marked with *)
        emailList.forEach((email, index) => {
          if (email.includes('*')) {
            correspondingAuthors.push(index);
          }
        });
      }
    }
  ...
  // Ensure all arrays have the same length as names (pad with empty strings if needed)
  while (departments.length < names.length) {
    departments.push('');
  }
  while (emails.length < names.length) {
    emails.push('');
  }
...
const analyzeDocxStructure = (html: string, rawText: string, ommlEquations: Equation[], file: File): Omit<Analysis, 'rawHtml' | 'rawText' | 'fileName' | 'fileSize' | 'fileType'> & { sectionEquationMap: Map<number, number[]> } => {
  ...
  const analysis = {
    ...
    authorInfo: null as AuthorInfo | null,
...
const detectDocumentElements = (analysis: {
  ...
  // Enhanced Author detection - try structured format first, then fallback to simple
  if (analysis.title && titleCandidate) {
    // Method 1: Look for structured author format (Name:, Department:, University:, Mail:)
    let structuredAuthorText = '';
    let collectingAuthorBlock = false;
    
    for (let i = titleCandidate.index + 1; i < Math.min(analysis.textLines.length, titleCandidate.index + 20); i++) {
      const line = analysis.textLines[i];
      const lowerLine = line.fullText.toLowerCase();
      
      // Start collecting if we find author-related keywords
      if (lowerLine.startsWith('name') || lowerLine.startsWith('author') || 
          lowerLine.startsWith('department') || lowerLine.startsWith('university') || 
          lowerLine.startsWith('mail') || lowerLine.startsWith('email')) {
        if (!collectingAuthorBlock) {
          collectingAuthorBlock = true;
        }
        structuredAuthorText += line.fullText + '\n';
      }
      // Continue collecting if we're in an author block and find related content
      else if (collectingAuthorBlock && (lowerLine.includes('cse') || lowerLine.includes('it') || 
               lowerLine.includes('@') || lowerLine.includes('university') || 
               lowerLine.includes('department') || lowerLine.includes('college'))) {
        structuredAuthorText += line.fullText + '\n';
      }
      // Stop collecting if we hit a section or abstract
      else if (collectingAuthorBlock && (line.startsWithNumber || line.containsAbstract || 
               lowerLine.startsWith('abstract') || lowerLine.startsWith('introduction'))) {
        break;
      }
      // If we've started collecting but this line doesn't seem related, stop
      else if (collectingAuthorBlock && line.fullText.trim().length > 10) {
        break;
      }
    }
    
    // Try to parse structured author information
    if (structuredAuthorText.trim()) {
      console.log('🔍 Attempting structured author parsing:', structuredAuthorText);
      const parsedAuthorInfo = parseAuthorInfo(structuredAuthorText);
      if (parsedAuthorInfo) {
        analysis.authorInfo = parsedAuthorInfo;
        analysis.authors = {
          text: `${parsedAuthorInfo.names.join(', ')} (${parsedAuthorInfo.names.length} authors)`,
          confidence: 0.95,
          reasoning: 'Structured author format detected with semicolon separation'
        };
        console.log('✅ Successfully parsed structured authors:', parsedAuthorInfo);
      }
    }
    
    // Method 2: Fallback to simple author detection if structured parsing failed
    if (!analysis.authors) {
      const authorCandidate = analysis.textLines.find((line: TextLine) => 
        line.index > titleCandidate.index && 
        line.index < titleCandidate.index + 5 &&
        line.fullText.length > 2 &&
        line.fullText.length < 100 &&
        !line.containsAbstract &&
        !line.startsWithNumber &&
        !line.isInTable &&
        !line.fullText.toLowerCase().includes('university') &&
        !line.fullText.toLowerCase().includes('department') &&
        !line.fullText.toLowerCase().startsWith('name') &&
        !line.fullText.toLowerCase().startsWith('mail')
      );
      ...
      if (authorCandidate) {
        analysis.authors = {
          ...
          confidence: 0.65,
          reasoning: 'Simple author format detected (single line after title)'
        };
        console.log('📝 Using simple author format:', authorCandidate.fullText);
      }
    }
  }
